    ...
    return RunRecord(
        ...
        generations=[
            Generation(
                index=0,
                variants=[
                    Variant(genome=champion, fitness=0.92, cost_est=0.014, p50_latency_ms=180),
...
def test_build_routing_card_skips_unfinished_runs():
    card = build_routing_card("legal", [RunRecord(run_id="unfinished", task_id="unknown", seed=1337)])
