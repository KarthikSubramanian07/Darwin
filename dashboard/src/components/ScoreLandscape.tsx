...
//
// Winner uses the design system's own --primary (#4f7bff), so a winner reads identically here
// and in the grid. Validated against the dashboard surface (#111113) with the dataviz
// validator: #4f7bff and failed #d03b3b pass lightness band, chroma floor, CVD separation
// (all-pairs protan dE 29.5), normal-vision floor (33.8) and 3:1 contrast. The neutral is
...
const WINNER = "#4f7bff"; // === --primary, the same accent the grid marks winners with
const FAILED = "#d03b3b";
const NEUTRAL = "#52525b"; // zinc, deliberately de-chromatised: "grey the rest"
const PENDING = "#27272a";
  ...
  return (
    <group position={[0, y, 0]}>
      <mesh rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshBasicMaterial color="#a1a1aa" transparent opacity={0.1} depthWrite={false} />
      </mesh>
      <lineSegments>
        <edgesGeometry args={[new PlaneGeometry(w, d).rotateX(-Math.PI / 2)]} />
        <lineBasicMaterial color="#c4c4cd" transparent opacity={0.55} />
      </lineSegments>
    </group>
  );
}
...
function Scene({
  ...
  return (
    <>
      ...
      <gridHelper
        args={[Math.max(width, depth) + SPACING, Math.max(nCols, nRows) + 1, "#2a2a30", "#1e1e23"]}
        position={[0, 0, 0]}
      />
